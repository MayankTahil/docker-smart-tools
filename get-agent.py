...
class st_agent:
    ...
    def __init__(self):
        ...
        self.output = os.path.dirname(os.path.realpath(__file__)) + '/install_agent.bin'
        self.debug = False
    ...
    def get_oAuthToken(self):
        ...
        url = make_url("https://smart.cloud.com/v0/roles?", { 'client_id' : self.apikey, 'company_id' : self.companyid })
        self.role = json.loads(requests.request("GET", url).text)[0]
        if self.debug:
            debug(self.role)
        
        # Get OAuth Token
        url = make_url("https://smart.cloud.com/v0/oauth/token?", { 'grant_type' : 'client_credentials', 'scope' : self.role + "," + self.companyid})
